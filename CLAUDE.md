...
`start.sh` runs **two separate Liquibase changelogs** before starting Tomcat: `liquibase.xml` against the **OpenMRS** database, and `liquibase_bahmni_reports.xml` against this service's **own** database — both via `package/resources/run-liquibase.sh`, a plain `#!/bin/sh` script that builds a `java -cp` classpath by hand from jars inside the exploded WAR. That script currently hardcodes exact jar filenames (`liquibase-core-4.8.0.jar`, `mysql-connector-java-8.0.28.jar`) rather than deriving them, which breaks silently on any Liquibase/connector version bump — see the open upgrade plan below.
...
- `reports_integration_tests` — an OpenMRS-shaped schema, from `openmrs_schema.sql`. **This is a hand-maintained, disposable fixture, not a verified copy of a real OpenMRS database** — it has been patched more than once to add columns/tables that a specific openmrs-api version's Hibernate mappings require, only when something happened to trip over the gap. It can still not match what a real OpenMRS deployment looks like in ways nothing in this suite would catch, because none of the 38 report SQL files under `src/main/resources/sql/` are exercised against it as part of the suite today.
...
`docs/upgrade/2.8.9-upgrade-plan.md` (plus `docs/upgrade/evidence/*.md`) is an active, checkpointed plan moving the test-scope `openmrs-api` dependency from 2.5.7 to 2.8.9, along with the Liquibase version it ships and the `run-liquibase.sh` fragility mentioned above. It documents, with evidence, real forced changes between those versions (a live Docker daemon becoming a hard test-time dependency, OpenMRS's DBUnit harness hardcoding a database name, schema columns added mid-range, dependency version floors) that are easy to rediscover the hard way. **Read it before touching anything related to the OpenMRS version, Liquibase, the JDK version, or the test harness** — it is the source of truth for those decisions, not something to re-derive from scratch.

SELECT
  ...
  sum(CASE WHEN
    first_concept.answer IS NOT NULL AND second_concept.answer IS NOT NULL AND third_concept.answer IS NOT NULL AND
    p.gender IS NOT NULL THEN 1
...
FROM
  ...
  LEFT OUTER JOIN (
    ...
    ON third_concept.answer = third_answers.answer
       ...
       AND first_concept.visit_id = third_concept.visit_id
  LEFT OUTER JOIN person p ON first_concept.person_id = p.person_id AND p.gender = gender.gender
    ...
    DAY)) AND (DATE_ADD(DATE_ADD(p.birthdate, INTERVAL rag.max_years YEAR), INTERVAL
                        rag.max_days DAY))
GROUP BY first_answers.answer_name, second_answers.answer_name, third_answers.answer_name, gender.gender, rag.name,
  rag.sort_order;
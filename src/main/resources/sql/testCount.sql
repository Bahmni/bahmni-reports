...
FROM test_section ts
  ...
  LEFT OUTER JOIN result r ON a.id = r.analysis_id
  LEFT OUTER JOIN result r1 ON a.id = r1.analysis_id AND r1.result_type = 'D'
...
where r.lastupdated  BETWEEN '%s' and '%s'

SELECT
  ...
  sum(CASE WHEN a.visit_id IS NOT NULL AND a.value_reference IS NOT NULL THEN 1
...
FROM
  (SELECT DISTINCT
   ...
   FROM
     ...
     INNER JOIN (SELECT DISTINCT value_reference AS type
                 FROM visit_attribute) visit_type %s
  ...
  reporting_age_group.max_days DAY)) AND cast(visit.date_started AS DATE) BETWEEN '%s' AND '%s'

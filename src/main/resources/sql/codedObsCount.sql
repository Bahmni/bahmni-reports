SELECT
  ...
  sum(IF(person.person_id IS NOT NULL AND attr.visit_attribute_id IS NOT NULL AND person.gender = 'F', 1, 0)) AS female_count,
  sum(IF(person.person_id IS NOT NULL AND attr.visit_attribute_id IS NOT NULL AND person.gender = 'M', 1, 0)) AS male_count,
  sum(IF(person.person_id IS NOT NULL AND attr.visit_attribute_id IS NOT NULL AND person.gender = 'O', 1, 0)) AS other_count,
  sum(IF(person.person_id IS NOT NULL AND attr.visit_attribute_id IS NOT NULL, 1, 0))                         AS total_count
FROM
  ...
  INNER JOIN (SELECT distinct value_reference as type from visit_attribute) visit_type %s
  ...
  group by age_group, concept_name, visit, answer_concept_name, sort_order

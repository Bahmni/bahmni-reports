...
from
  ...
  (select cn.name as concept_name,
   ...
   from obs obs
     ...
     inner join reporting_age_group rag ON obs_datetime BETWEEN (DATE_ADD(DATE_ADD(birthdate, INTERVAL rag.min_years YEAR), INTERVAL rag.min_days DAY)) AND (DATE_ADD(DATE_ADD(birthdate, INTERVAL rag.max_years YEAR), INTERVAL rag.max_days DAY)) and rag.report_group_name="Inpatient Discharge Reports"
